...
export const MultiMonitorsControlsManager = GObject.registerClass(
    class MultiMonitorsControlsManager extends St.Widget {
        _init(index, settings) {
            ...
            this._pageEmptyId = 0;
            const hasSignal = (obj, name) => {
                try {
                    const gtype = obj?.constructor?.$gtype;
                    return gtype && GObject.signal_lookup(name, gtype) !== 0;
                } catch (_e) {
                    return false;
                }
            };
            if (Main.overview.searchController && typeof Main.overview.searchController.connect === 'function') {
                try {
                    if (hasSignal(Main.overview.searchController, 'page-changed')) {
                ...
                try {
                    if (hasSignal(Main.overview.searchController, 'page-empty')) {
